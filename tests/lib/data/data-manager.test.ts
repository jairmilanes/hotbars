...
import { cliDefaults, Config } from "hotbars/lib/core";
import { DataManager } from "hotbars/lib/data"
import { CliOptions } from "hotbars/types";
import { QueryBuilder } from "hotbars";
...
describe("Data Manager", () => {
  ...
  beforeAll(async () => {
    ...
    const jsonRouter = jsonServer.router<any>(Config.get("jsonDb"));
    manager = await DataManager.create("lowDb", jsonRouter.db);
  })

  test("should return an user by id", async () => {
    const user = await manager.from("users").get("4a70a406-fd31-4adb-8fcd-a90cd4eff556")
    expect(user.id).toEqual("4a70a406-fd31-4adb-8fcd-a90cd4eff556");
  })

  test("should find user whenre id equals id", async () => {
    const user = await manager.from("users").eq("id", "4a70a406-fd31-4adb-8fcd-a90cd4eff556").single()
    expect(user.id).toEqual("4a70a406-fd31-4adb-8fcd-a90cd4eff556");
  })

  test("should return users for neq query", async () => {
    const users = await manager.from("users").neq("id", "4a70a406-fd31-4adb-8fcd-a90cd4eff556").all()
    expect(users.every((u: Record<string, any>) => u.id !== "4a70a406-fd31-4adb-8fcd-a90cd4eff556")).toEqual(true);
  })
  ...
  test("should return users for gte query", async () => {
    ...
    expect(users.every((u: Record<string, any>) => u.age >= 44)).toEqual(true);
  })

  test("should return users for lte query", async () => {
    const users = await manager.from("users").lte("age", 24).all()
    expect(users.every((u: Record<string, any>) => u.age <= 24)).toEqual(true);
  })

  test("should return users for in query", async () => {
    const users = await manager.from("users").in("age", [24, 19, 39]).all()
    expect(users.every((u: Record<string, any>) => [24, 19, 39].indexOf(u.age) !== -1)).toEqual(true);
  })
  ...
  test("should update records where", async () => {
    const db = manager.from("users")
    const rows = await db.updateWhere({gender: "Masculino"}, { countryCode: "US" });
    expect(rows.every(r => r.countryCode === "US")).toBeTruthy();
    const originals = await db.updateWhere({gender: "Masculino"}, { countryCode: "BR" });
